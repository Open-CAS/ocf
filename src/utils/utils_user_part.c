...
void ocf_user_part_init(struct ocf_cache *cache)
{
	...
	for (i_part = 0; i_part < OCF_USER_IO_CLASS_MAX + 1; i_part++)
		cache->user_parts[i_part].part.id = i_part;
...
void ocf_user_part_load(struct ocf_cache *cache)
{
	ocf_part_id_t i_part;
	...
	for (i_part = 0; i_part < OCF_USER_IO_CLASS_MAX; i_part++) {
		user_part = &cache->user_parts[i_part];

		ocf_refcnt_freeze(&user_part->cleaning.counter);

		ENV_BUG_ON(ocf_mngt_add_partition_to_cache(cache,
				i_part, user_part->config->name,
				user_part->config->min_size,
				user_part->config->max_size,
				user_part->config->priority,
