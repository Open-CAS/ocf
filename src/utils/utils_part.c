...
uint32_t ocf_part_overflow_size(struct ocf_cache *cache, struct ocf_user_part *part)
{
	...
	if (part_occupancy > part_occupancy_limit)
		return part_occupancy_limit - part_occupancy;
...
bool ocf_part_has_space(struct ocf_request *req)
{
	...
	uint32_t needed_cache_lines = ocf_engine_repart_count(req) +
		ocf_engine_unmapped_count(req);
	uint32_t part_occupancy = ocf_part_get_occupancy(target_part);
