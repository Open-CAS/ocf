...
void ocf_req_hash(struct ocf_request *req)
{
	...
	uint core_line_count = req->core_line_count;
...
void ocf_io_end_func(ocf_io_t io, int error)
{
	...
	req->io.chkpts[DEBUG_CHKPT_COMPLETE] = env_get_tick_count();
#endif
	if (req->io.end)
...
void ocf_req_forward_cache_io_func(struct ocf_request *req, int dir, uint64_t addr,
