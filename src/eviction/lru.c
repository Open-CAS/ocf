...
static inline void lru_iter_init(struct ocf_lru_iter *iter, ocf_cache_t cache,
		...
		bool cl_lock_write, bool hash_lock_write, bool exclusive,
		_lru_hash_locked_pfn hash_locked, void *context)
{
	...
	iter->hash_lock_write = hash_lock_write;
...
static inline void lru_iter_cleaning_init(struct ocf_lru_iter *iter,
		...
		uint32_t start_evp)
{
	/* Lock cachelines and hash bucket for read only, non-exclusive access,
	   nothing locked by the caller */
	lru_iter_init(iter, cache, part, start_evp, false, false, false, false,
			NULL, NULL);
}
...
{
	/* Lock hash buckets for write, cachelines according to user requet,
	...
	lru_iter_init(iter, cache, part, start_evp, true, cl_lock_write,
			true, true, _evp_lru_evict_hash_locked, req);
}
		...
		ocf_cache_line_t cline)
{
	return iter->cl_lock_write ?
		ocf_cache_line_try_lock_wr(iter->cache, cline) :
		ocf_cache_line_try_lock_rd(iter->cache, cline);
}
		...
		ocf_cache_line_t cline)
{
	if (iter->cl_lock_write)
		ocf_cache_line_unlock_wr(iter->cache, cline);
	else
		ocf_cache_line_unlock_rd(iter->cache, cline);
}
...
static bool inline _lru_lock(struct ocf_lru_iter *iter,
		...
		ocf_core_id_t *core_id, uint64_t *core_line)
{
	...
	do {
		...
		ocf_metadata_eviction_lock(&cache->metadata.lock, curr_evp);

		list = evp_lru_get_list(part, curr_evp, iter->clean);

		cline = list->tail;
		while (cline != end_marker &&
				!_lru_lock(iter, cline, core_id, core_line)) {
		...
		ocf_metadata_eviction_unlock(&cache->metadata.lock, curr_evp);
...
static void evp_lru_clean_end(void *private_data, int error)
{
	...
	struct ocf_lru_iter *iter = &ctx->iter;
	unsigned i;

	for (i = 0; i < OCF_EVICTION_CLEAN_SIZE; i++) {
		if (ctx->data[i].cache_line != end_marker)
			_lru_unlock_cacheline(iter, ctx->data[i].cache_line);
	}

	ocf_refcnt_dec(&iter->part->cleaning.counter);
}
		...
		uint32_t idx, ocf_cache_line_t *line)
{
	...
	struct ocf_lru_iter *iter = &ctx->iter;
	struct flush_data *data = &ctx->data[idx];

	data->cache_line = lru_iter_cleaning_next(iter, &data->core_id,
			&data->core_line);
	if (data->cache_line == end_marker)
		return -1;

	ENV_BUG_ON(!metadata_test_dirty(cache, data->cache_line));

	*line = data->cache_line;
	return 0;
}
		...
		uint32_t idx)
{
	...
	_lru_unlock_hash(iter, data->core_id, data->core_line);
}

void evp_lru_clean(ocf_cache_t cache, struct ocf_user_part *part,
		ocf_queue_t io_queue, uint32_t count)
{
	struct ocf_refcnt *counter = &part->cleaning.counter;
	...
	lru_iter_cleaning_init(&part->cleaning.iter, cache, part,
			part->cleaning.iter.evp);
	for (i = 0; i < OCF_EVICTION_CLEAN_SIZE; i++)
		part->cleaning.data[i].cache_line = end_marker;
		...
		struct ocf_user_part *part, uint32_t cline_no)
{
	...
	evp = env_atomic64_inc_return(&part->curr_evp) %
	...
	while (i < cline_no) {
		...
		ocf_engine_update_req_info(cache, req, req_idx);
