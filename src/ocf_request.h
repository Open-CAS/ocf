...
enum {
	DEBUG_CHKPT_ALLOC,
	DEBUG_CHKPT_SUBMIT,
	DEBUG_CHKPT_COMPLETE,
	DEBUG_CHKPT_PUSH_PRIO_HIGH,
	DEBUG_CHKPT_PUSH_PRIO_LOW,
	DEBUG_CHKPT_POP,
...
struct ocf_request_io {
	...
	/**
	 * @brief Prefetch request: 0 - not prefetch, !0 - prefetch algorithm id
	 */
	pf_algo_id_t pa_id:OCF_PA_ID_MAX_BITS;
	...
	/**
	 * @brief OCF blktrace Support
	 */
	OCF_BLKTRACE_DECLARE(ocf_blktrace_io_t, ocf_io_blktrace);
...
#ifdef OCF_DEBUG_STATS
	uint64_t chkpts[DEBUG_CHKPT_LAST];
#endif
};
...
struct ocf_request {
	...
	struct ocf_req_ocf_counters_cache_feedback ocf_cache_feedback_counters;
	/*!< Request processing counters, incremented per cacheline */
...
void ocf_req_forward_cache_io_func(struct ocf_request *req, int dir,
		uint64_t addr, uint64_t bytes, uint64_t offset);

#define ocf_req_forward_cache_io(_req, _dir, _addr, _bytes, _offset)	\
	do {								\
		OCF_BLKTRACE_REMAP_TO_CACHE(_req, _addr, _dir);		\
		ocf_req_forward_cache_io_func(_req, _dir, _addr,	\
						_bytes, _offset);	\
	} while (0);
...
void ocf_req_set_lower_cache_mode(struct ocf_request *req,
