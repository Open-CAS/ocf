...
static struct ocf_request *ocf_queue_pop_req(ocf_queue_t q)
{
	...
	req->io.chkpts[DEBUG_CHKPT_POP] = env_get_tick_count();
#endif

	OCF_CHECK_NULL(req);
	OCF_BLKTRACE_REQ_Q_POP(req);
...
#ifdef OCF_DEBUG_STATS
	req->io.chkpts[(flags & OCF_QUEUE_PRIO_HIGH)
			? DEBUG_CHKPT_PUSH_PRIO_HIGH
			: DEBUG_CHKPT_PUSH_PRIO_LOW] = env_get_tick_count();
#endif
