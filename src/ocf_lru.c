...
static const ocf_cache_line_t end_marker = (ocf_cache_line_t)(OCF_CACHE_LINE_NUM - 1);
		...
		_lru_hash_locked_pfn hash_locked, struct ocf_request *req)
{
	...
	ENV_BUILD_BUG_ON(OCF_NUM_LRU_LISTS > sizeof(iter->next_avail_lru) * __CHAR_BIT__);
...
static bool inline _lru_iter_eviction_lock(struct ocf_lru_iter *iter,
...
static inline ocf_cache_line_t lru_req_next_cline(struct ocf_request *req,
	ocf_cache_t cache, struct ocf_lru_iter *iter, ocf_cache_line_t cline,
	struct ocf_part *dst_part, ocf_core_id_t *core_id, uint64_t *core_line,
		...
		uint64_t *core_line, uint32_t *prev_lru)
{
	...
	do {
		...
		if (cline != end_marker) {
			if (dst_part != part) {
				/* evict cline, reassign from part to dst_part */
				ocf_lru_repart_locked(cache, cline, part,
						dst_part);
			} else {
				/* evict cline, prepare for new data on same partition */
		...
		struct ocf_part *src_part, uint32_t cline_no)
{
	...
	/* evicted cache-line's associated core, algorithm and size */
	ocf_core_t core;
	pf_algo_id_t pa_id;
...
got_cline:
		...
		if (actual_src_part_id != PARTITION_FREELIST) {
			pa_id = ocf_metadata_get_algorithm_id(cache, cline);

			ocf_lru_invalidate(cache, cline, core_id,
				actual_src_part_id);
			_lru_unlock_hash(&iter, core_id, core_line);

			/* count evicted data as (FP) (false-positive) feedback for content-management
			 * algorithm.
			 *
			 * NOTE: even though we may have given false-positive feedback at passing
			 *  mid-point, we can give more bad-feedback, it's even worse if data was
			 *  finally evicted without being read.
			 */
			core = ocf_cache_get_core(cache, core_id);

			/* count for prefetchers or admission */
			ocf_cache_feedback_counters_req_cache_evicted_blocks_inc(core, req, pa_id);
		}
		...
		void *priv, unsigned shard_id, unsigned shards_cnt)
{
	...
	struct ocf_lru_list *list;
	unsigned lru_list = lruid_to_bitreversal[shard_id];
	unsigned step = 0;
	...
	for (i = 0; i < portion; i++) {
		...
		cline = ocf_get_cline_by_lru(entries, shards_cnt, shard_id, i);
